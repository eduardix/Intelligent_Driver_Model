...
lambda1 = 1; %1 coche por lambda s lane 1 
lambda2 = 1; %1 coche por lambda s lane 2

llegadas1 = zeros(1,numel(0:dt:tmax));
llegadas2 = zeros(1,numel(0:dt:tmax));
cambios = zeros(1,numel(0:dt:tmax));
...
I     = V0 <= v0-2*desv | V0 > v0+desv;
V0(I) = v0; %Y que la velocidad mínima no va a ser menor que mu-n*sigma

%Variables de salida y control CARRIL 1 y 2:
...
%parámetros MOBIL
global p b_mobil ath gap_t gap_d;
...
for t = 0:dt:tmax
    ...
    if semaforo 
       if t_semaforo < t_rojo
           t_semaforo = t_semaforo + dt;
       else
           semaforo = 0;
           t_semaforo = dt;
           A1(A1(:,7) == -10,:) = [];
       ...
       if t_semaforo < t_verde
           t_semaforo = t_semaforo + dt;
       else
           semaforo = 1;
           t_semaforo = dt;
           A1 = [pos_semaforo t 0 0 0 0 -10 0 0 0 0 0 0 1 1; A1];
           A1 = sortrows(A1,1);
       end
    end

    %Paso de simulación IDM + MOBIL
    [A1,A2,cambios(c)] = MIXED_engine(A1,A2,dt);
    c = c+1;
...
%% Ahora el colores
figure
scatter(stv_lane1(:,2),stv_lane1(:,1),5,stv_lane1(:,3),'filled');
axis([0 param.tmax 0 param.smax]);
caxis([0 33]);
xlabel('time (min)','fontsize',14);
ylabel('space (km)','fontsize',14);
...
figure
scatter(stv_lane2(:,2),stv_lane2(:,1),5,stv_lane2(:,3),'filled');
axis([0 param.tmax 0 param.smax]);
caxis([0 33]);
xlabel('time (min)','fontsize',14);
ylabel('space (km)','fontsize',14);
box on
title('Lane 2');
%% Película (como una carretera)
figure
for t = 400:param.dt:param.tmax
